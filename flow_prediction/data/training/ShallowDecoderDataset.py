...
def _sensor_mean_center(sensor_values, full_field_values):
    ...
    return sensor_values-means, full_field_values-means
...
def _extract_values(case, sensor_slice=None, full_field_slice=None, normalization=None):
    normalization_methods = {'sensor_mean_center': _sensor_mean_center, 'full_field_mean_center': _fullfield_mean_center, None: lambda x,y: (x,y)}
    ...
    sensor_values, full_field_values = normalization_methods[normalization](np.array(case['sensor_values']), np.array(case['full_field_values']))
    ...
    return sensor_values, full_field_values

def ShallowDecoderDataset(h5path, batch_size=None, shuffle=False, retain_time_dimension=False, sensor_masks = None, full_field_mask = None, h5py_driver = None, h5py_driver_kwargs = None, normalization = None, dtypes = None, train_test_split = None, rand_seed = None):
    ...
    sensor_values, full_field_values = zip(*[_extract_values(h5file[case], sensor_slice = sensor_slice, full_field_slice=full_field_slice, normalization=normalization) for case in h5file.keys()])
    ...
    full_field_values = full_field_values.astype(dtypes[0])
    
    if train_test_split is None:
        dset = tf.data.Dataset.from_tensor_slices((sensor_values,full_field_values))
        if shuffle:
            dset = dset.shuffle()
        ...
        dset_train = tf.data.Dataset.from_tensor_slices((train_sensor, train_fullfield))
        dset_test = tf.data.Dataset.from_tensor_slices((test_sensor, test_fullfield))
