...
class ShallowDecoderPI(tf.keras.Sequential):
    ...
    def __init__(self, NS_annulus_residual_driver, target_var_order, output_var_order, *sd_args, other_loss = None, other_loss_weight = 1.0, PI_loss_weight = 1.0, optimizer = None, data_normalization = None, normalization_var_order = None, metrics = None, **sd_kwargs):
        layers = _get_original_shallow_decoder_layers(*sd_args, **sd_kwargs)
        ...
        #self._PILoss_target_scatter_indices = tf.constant(PILoss_target_scatter_indices, tf.int32)
        #self._PILoss_output_scatter_indices = tf.constant(PILoss_output_scatter_indices, tf.int32)
    ...
    def extract_other_loss_inputs(self, targets, pred):
        ...
        targets = tf.gather(tf.transpose(targets,[2,0,1]),self._target_indices_of_output_vars)
        targets = tf.transpose(targets, [1,2,0])
